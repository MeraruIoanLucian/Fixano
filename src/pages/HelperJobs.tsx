...
export default function HelperJobs() {
    ...
    // filtram client-side — status tab
        ...
        <div style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
            <main className="pt-12 pb-24 min-h-screen px-6 md:px-12 max-w-screen-2xl mx-auto">
                <header className="mb-12">
                    <SectionHeading title="Your Jobs" subtitle="View and manage your active projects." />
                </header>
                {/* Tab Filter Bar */}
                ...
                {!loading && !error && filteredJobs.length === 0 && (
                    <EmptyState
                        title={activeTab === 'all' ? 'No jobs available yet' : `No ${TABS.find(t => t.value === activeTab)?.label.toLowerCase()} jobs`}
                        description={activeTab === 'all' ? "Couldn't find any job posts matching your criteria." : 'Try selecting a different tab.'}
                    />
                )}
